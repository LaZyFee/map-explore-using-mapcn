...
function ControlGroup({
  ...
  return (
    <div
      className="border-border  group [&>button:not(:last-child)]:border-border flex flex-col overflow-hidden rounded-md border shadow-sm [&>button:not(:last-child)]:border-b">
      {children}
    </div>
  );
}
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "hover:bg-accent dark:hover:bg-accent/40 group flex size-8 items-center justify-center transition-colors",
      ...
      {showZoom && (
        <ControlGroup>
          <ControlButton onClick={handleZoomIn} label="Zoom in">
            <Plus className="size-4 text-white group-hover:text-black" />
          </ControlButton>
          <ControlButton onClick={handleZoomOut} label="Zoom out">
            <Minus className="size-4 text-white group-hover:text-black" />
          </ControlButton>
        </ControlGroup>
      )}
      {showCompass && (
      ...
      {showLocate && (
        <ControlGroup>
          <ControlButton
            ...
            {waitingForLocation ? (
              <Loader2 className="size-4 animate-spin text-white group-hover:text-black" />
            ) : (
              <Locate className="size-4 text-white group-hover:text-black" />
            )}
          </ControlButton>
        </ControlGroup>
      )}
      {showFullscreen && (
        <ControlGroup>
          <ControlButton onClick={handleFullscreen} label="Toggle fullscreen">
            <Maximize className="size-4 text-white group-hover:text-black" />
          </ControlButton>
        </ControlGroup>
      )}
    </div>
